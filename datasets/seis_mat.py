...
class SeisMatTrainDataset_clean(BaseTrainDataset):
    def __init__(self, path, patch_size, pin_memory):
        super(SeisMatTrainDataset_clean, self).__init__(path, patch_size, pin_memory)
    ...
    def __getitem__(self, index):
        ...
        img_H= self.crop(img_H)
        img_H= self.augment(img_H)
    ...
    def _get_img_paths(self, path):
        self.img_paths = []
        H_pattern = os.path.join(path, 'CL/*.mat')
        H_paths = sorted(glob.glob(H_pattern))
        for H_path in H_paths:
            self.img_paths.append({'H': H_path})
        # self.img_paths = self.img_paths[:int(30 / 40 * len(self.img_paths))]#mcj
        # self.img_paths = self.img_paths[int(39 / 40 * len(self.img_paths)):int(40 / 40 * len(self.img_paths))]  # mcj
        self.img_paths = self.img_paths[:]  # mcj
...
class SeisMatValidationDataset_clean(BaseTrainDataset):
    def __init__(self, path, patch_size, pin_memory):
        super(SeisMatValidationDataset_clean, self).__init__(path, patch_size, pin_memory)
        self._get_img_paths(path)
        self.n_data = len(self.img_paths)
